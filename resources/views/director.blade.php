...
<div class="">
	<div class="mx-5 mt-3 mb-4">
		...
		<div class="row mb-3 tab-pane fade show active" id="employee_table">
			<div class="top-two">
				...
				<div>
					<a href="{{route('director.create')}}" class="btn btn-success float-end">Add New <i class="fas fa-user-plus"></i></a>
				</div>
				...
				<table class="table table-hover" id="test">
					<thead>
						<tr>
							...
							<th style="width: 150px;"><span class="mm">ထုတ်ယူမှုပုံစံ</span><span class="eng">Extract <br> Form</span></th>
	...
	<script>
		...
		$.ajax({
			type: "GET",
			url: "{{ route('director.list') }}",
			data: data,
			dataType: "JSON",
			success: function (response) {
				var output = '';
				$.each(response, function (index, value) {
					let file = value.formc_filename;
					if(file){
						file = value.formc_filename;
					}else{
						file = '';
					}
					...
					output += `
						<tr style="vertical-align: middle;">
                            ...
                            <td>
								${value.extract_filename == null ?
									`<label class="badge badge-danger">No Have</label>`
									:`<a href='public/${value.extract_filename}'>Yes</a>`}                           
                            </td>							
							<td>
								${value.status == 1 ? 
							...
							<td>
								<a href="/director_edit/${value.id}" class="btn btn-primary" style="min-width:120px">Edit<i class="fas fa-user-edit"></i></a>
								<a href="/director_delete/${value.id}">
